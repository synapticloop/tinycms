...
import com.synapticloop.tinycms.sqlite.h2zero.model.FieldType;

public class FieldTypeFinder {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(FieldTypeFinder.class);
	private static final String SQL_SELECT_START = 
		"""
			select 
				id_field_type, 
				nm_field_type, 
				fl_field_should_quote
			from 
				field_type
		""";
	private static final String SQL_BUILTIN_FIND_BY_PRIMARY_KEY = SQL_SELECT_START + " where id_field_type = ?";

	// now for the statement limit cache(s)
	private static final LruCache<String, String> findAll_limit_statement_cache = new LruCache<>(1024);

	private FieldTypeFinder() {}

	/**
	 * Find a FieldType by its primary key
	 * 
	 * @param connection the connection item
	 * @param idFieldType the primary key
	 * 
	 ...
	 */
	public static FieldType findByPrimaryKey(Connection connection, Long idFieldType) throws H2ZeroFinderException {
		FieldType fieldType = null;
		PreparedStatement preparedStatement = null;
		ResultSet resultSet = null;

		if(null == idFieldType) {
			throw new H2ZeroFinderException("Could not find result as the primary key field [idFieldType] was null.");
		}

		try {
			preparedStatement = connection.prepareStatement(SQL_BUILTIN_FIND_BY_PRIMARY_KEY);
			preparedStatement.setLong(1, idFieldType);
			resultSet = preparedStatement.executeQuery();
			fieldType = uniqueResult(resultSet);
		} catch (H2ZeroFinderException | SQLException ex) {
			throw new H2ZeroFinderException(ex.getMessage() + "  Additionally, the parameters were [idFieldType:" + idFieldType + "].");
		} finally {
			ConnectionManager.closeAll(resultSet, preparedStatement);
		}

		if(null == fieldType) {
			throw new H2ZeroFinderException("Could not find result the parameters were [idFieldType:" + idFieldType + "].");
		}
		return(fieldType);
	}
	 ...
	 * @throws H2ZeroFinderException if one couldn't be found
	 */
	public static FieldType findByPrimaryKey(Long idFieldType) throws H2ZeroFinderException {
		...
		try (Connection connection = ConnectionManager.getConnection()) {
			fieldType = findByPrimaryKey(connection, idFieldType);
	...
	/**
	 * Find a FieldType by its primary key and silently fail.
	 ...
	 * 
	 */
	public static FieldType findByPrimaryKeySilent(Connection connection, Long idFieldType) {
		try {
			return(findByPrimaryKey(connection, idFieldType));
		} catch(H2ZeroFinderException h2zfex){
			if(LOGGER.isWarnEnabled()) {
				LOGGER.warn("H2ZeroFinderException findByPrimaryKeySilent(" + idFieldType + "): " + h2zfex.getMessage());
	 ...
	 * 
	 */
	public static FieldType findByPrimaryKeySilent(Long idFieldType) {
		try {
			return(findByPrimaryKey(idFieldType));
		} catch(H2ZeroFinderException h2zfex){
			if(LOGGER.isWarnEnabled()) {
				LOGGER.warn("H2ZeroFinderException findByPrimaryKeySilent(" + idFieldType + "): " + h2zfex.getMessage());
				if(LOGGER.isDebugEnabled()) {
	 ...
	 * 
	 * @return a list of all the FieldType objects
	 * 
	 * @throws SQLException if there was an error in the SQL statement
	 * @throws H2ZeroFinderException if no results were found
	 */
	public static List<FieldType> findAll(Connection connection, Integer limit, Integer offset) throws SQLException, H2ZeroFinderException {
	...
	/**
	 * Find all the FieldType objects - in effect this chains 
	 * to the findAll(connection, limit, offset) with null parameters.
	 * 
	 * @return The list of FieldType model objects
	 * 
	 * @throws SQLException if there was an error in the SQL statement
	 * @throws H2ZeroFinderException if no results were found
	 */
	public static List<FieldType> findAll() throws SQLException, H2ZeroFinderException {
	 ...
	 * @throws H2ZeroFinderException if no results were found
	 */
	public static List<FieldType> findAll(Connection connection) throws SQLException, H2ZeroFinderException {
	 ...
	 * @throws H2ZeroFinderException if no results were found
	 */
	public static List<FieldType> findAll(Integer limit, Integer offset) throws SQLException, H2ZeroFinderException {
	 ...
	 * @param offset - the offset from the start of the results
	 * 
	 * @return The list of FieldType model objects, or an empty List on error
	 */
	public static List<FieldType> findAllSilent(Connection connection, Integer limit, Integer offset) {
		try {
	 ...
	 * @return The list of FieldType model objects, or an empty List on error
	 */
	public static List<FieldType> findAllSilent(Connection connection) {
	 ...
	 * @return The list of FieldType model objects, or an empty List on error
	 */
	public static List<FieldType> findAllSilent(Integer limit, Integer offset) {
	 ...
	 * @return The list of FieldType model objects, or an empty List on error
	 */
	public static List<FieldType> findAllSilent() {
	 ...
	 * key.
	 * 
	 * There are 0 defined finders on the field_type table, of those finders
	 ...
	 * 
	 * @return The FieldType that represents this result
	 * 
	 * @throws H2ZeroFinderException if no results were found or more than one result was found
	 * @throws SQLException if there was a problem retrieving the results
	 */
	private static FieldType uniqueResult(ResultSet resultSet) throws H2ZeroFinderException, SQLException {
		if(resultSet.next()) {
			// we have a result
			Long idFieldType = ConnectionManager.getNullableResultLong(resultSet, 1);
			String nmFieldType = ConnectionManager.getNullableResultString(resultSet, 2);
			Boolean flFieldShouldQuote = ConnectionManager.getNullableResultBoolean(resultSet, 3);

			FieldType fieldType = new FieldType(idFieldType, nmFieldType, flFieldShouldQuote);
	...
	/**
	 * Return the results as a list of FieldType, this will be empty if
	 * none are found.
	 * 
	 * @param resultSet the results as a list of FieldType
	 * 
	 * @return the list of results
	 * 
	 * @throws SQLException if there was a problem retrieving the results
	 */
	private static List<FieldType> list(ResultSet resultSet) throws SQLException {
		List<FieldType> arrayList = new ArrayList<FieldType>();
		while(resultSet.next()) {
			arrayList.add(new FieldType(
