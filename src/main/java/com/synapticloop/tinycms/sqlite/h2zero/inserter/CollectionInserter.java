 ...
 *   <code>tinycms.collection</code>
 * table.  The fields that are available are as follows:
 * 
 * <ul>
 *  <li><code>id_collection</code> (bigint)  <strong>NOT</strong> nullable  (PRIMARY)</li>
...
public class CollectionInserter {
	...
	private static final String BINDER = Constants.COLLECTION_BINDER;
		...
		private static final Logger LOGGER = LoggerFactory.getLogger(CollectionInserter.class);

	// static fields generated by synapticloop h2zero
	private static final String SQL_BUILTIN_INSERT_ALL = "insert into collection(id_collection, nm_collection)";
	...
	private CollectionInserter() {}

	/**
	 * Insert a new Collection into the database utilising the passed in connection.
	 * 
	 ...
	 */
	public static int insert(Connection connection, Long idCollection, String nmCollection) throws SQLException {
	...
	/**
	 * Insert a new Collection into the database a new connection will be retrieved 
	 ...
	 * @throws SQLException if there was an error in the SQL insert statement
	 */
	public static int insert(Long idCollection, String nmCollection) throws SQLException {
		try (Connection connection = ConnectionManager.getConnection()) {
			return(insert(connection, idCollection, nmCollection));
		}
	}

	/**
	 * Silently (i.e. swallow any exceptions) Insert a new Collection into the 
	 ...
	 * @return the number of rows that were inserted, or -1 if an error occurred
	 */
	public static int insertSilent(Connection connection, Long idCollection, String nmCollection) {
		try {
			return(insert(connection, idCollection, nmCollection));
	 ...
	 * @return the number of rows that were inserted, or -1 if an error occurred
	 */
	public static int insertSilent(Long idCollection, String nmCollection) {
		try (Connection connection = ConnectionManager.getConnection()){
			return(insert(connection, idCollection, nmCollection));
		} catch (SQLException sqlex) {
			LOGGER.error("SQLException caught, message was: {}", sqlex.getMessage());
